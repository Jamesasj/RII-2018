...
for walk_length in range (1,31):
    ...
    for i in range(number_of_walks):
       ...
       distribuicao = GeraDistribuicao(mu,sigma,20)   
...
'''
  distribuicaoObservada = []
  n_alunos = 0
  
  for i in range(rodadas):
    distribuicao = GeraDistribuicao(53,6,20)
    dfDistribuicao = pd.DataFrame(distribuicao)
    IQR = dfDistribuicao.quantile(0.75) - dfDistribuicao.quantile(0.25) # > .85
    distribuicaoObservada.append(IQR)
    if float(IQR) > .85:      
      n_alunos += 1
    #print(len(distribuicaoObservada))
    #print(IQR)
    #print(distribuicao)
    distribuicaoObservada.append(IQR)
    #print (s)    
    print(dfDistribuicao.describe())
    #print('Quantil 50%:', dfDistribuicao.quantile(0.5))
  return distribuicaoObservada

mu, sigma = 53, 6
l = GeradorMonteCarlo(100)
